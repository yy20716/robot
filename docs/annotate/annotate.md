...
Annotations can be added one-by-one with `--anotation`, or all at once from a turtle file with `--annotation-file`. See <a href="/annotate/annotations.ttl" target="_blank">`annotations.ttl`</a> for an example.

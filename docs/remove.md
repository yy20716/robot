...
Multiple entities can be removed by providing the path to a text file containing each entity's CURIE on a separate line (see <a href="examples/remove/ids.txt" target="_blank">`ids.txt`</a> for an example):
```
